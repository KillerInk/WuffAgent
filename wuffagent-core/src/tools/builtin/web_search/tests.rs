...
const YAHOO_HTML_2026_09: &str = include_str!("../../../../tests/fixtures/yahoo-live-2026-09.html");
...
fn test_parse_bing_results_happy() {
    ...
    assert!(
        first["title"]
            .as_str()
            .unwrap_or("")
            .contains("Rust web framework comparison"),
        "unexpected title: {}",
        first["title"]
    );
...
fn test_clean_ddg_url() {
    ...
    assert_eq!(
        clean_ddg_url("https://plain.example/"),
        "https://plain.example/"
    );
}
...
fn test_run_chain_all_fail() {
    ...
    assert!(
        failures[1].starts_with("yahoo: no results (blocked)"),
        "{}",
        failures[1]
    );
    assert!(
