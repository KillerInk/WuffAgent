...
/// A general-purpose fallback worker with access to all available tools.
...
impl WorkerAgent for GeneralWorker {
    ...
    async fn execute_task(
        ...
        match result {
            Ok(ToolOutput::Success(output)) => {
                ...
                tracing::info!(
                    "GeneralWorker '{}' completed task '{}' in {}ms",
                    self.name,
                    task.description,
                    duration
                );
            ...
            Ok(ToolOutput::Error(err)) => {
                let duration = start.elapsed().as_millis() as u64;
                tracing::warn!(
                    "GeneralWorker '{}' task '{}' returned error: {}",
                    self.name,
                    task.description,
                    err
                );
