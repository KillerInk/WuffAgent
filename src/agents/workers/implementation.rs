...
/// A worker that executes implementation tasks using commands, builds, and deployments.
pub struct ImplementationWorker {
    id: AgentId,
    name: String,
    description: String,
...
impl WorkerAgent for ImplementationWorker {
    ...
    async fn execute_task(
        ...
        tracing::info!(
            "ImplementationWorker '{}' running task '{}'",
            self.name,
            task.description
        );
        ...
        match result {
            Ok(ToolOutput::Success(output)) => {
                ...
                tracing::info!(
                    "ImplementationWorker '{}' completed task '{}' in {}ms",
                    self.name,
                    task.description,
                    duration
                );
            ...
            Ok(ToolOutput::Error(err)) => {
                let duration = start.elapsed().as_millis() as u64;
                tracing::warn!(
                    "ImplementationWorker '{}' task '{}' returned error: {}",
                    self.name,
                    task.description,
                    err
                );
            ...
            Err(e) => {
                let duration = start.elapsed().as_millis() as u64;
                tracing::warn!(
                    "ImplementationWorker '{}' failed task '{}': {}",
                    self.name,
                    task.description,
                    e
                );
...
impl Agent for ImplementationWorker {
