...
impl AgentConfigDialog {
    ...
    pub fn view(&mut self, pal: Palette) -> Element<'static, Message> {
            ...
            for (i, agent) in self.agents.iter().enumerate() {
                ...
                col = col.push(
                    button(text(label).size(11).color(fg))
                        ...
                        .on_double_click(Message::AgentConfigEdit(i))
                        .padding([4, 8])
                        .style(iced::widget::button::Filled(bg, fg)),
                );
            }
            col.into()
        };

        let editor = if self.is_new || self.selected_index >= 0 {
            column!()
                .push(text(if self.is_new { "New Agent" } else { "Edit Agent" }).size(14).color(pal.text))
                .push(horizontal_rule(1).padding([4, 0]))
                .push(text("Name").size(11).color(pal.text))
                .push(text_input("", &self.name)
                    .on_input(|v| Message::AgentConfigName(v))
                    .padding([4, 8]))
                .push(text("Description").size(11).color(pal.text))
                .push(text_input("", &self.description)
                    .on_input(|v| Message::AgentConfigDescription(v))
                    .padding([4, 8]))
                .push(text("Priority (lower = preferred)").size(11).color(pal.text))
                .push(text_input("", &self.priority.to_string())
                    .on_input(|v| Message::AgentConfigPriority(v))
                    .padding([4, 8]))
                .push(text("Max Concurrent").size(11).color(pal.text))
                .push(text_input("", &self.max_concurrent.to_string())
                    .on_input(|v| Message::AgentConfigMaxConcurrent(v))
                    .padding([4, 8]))
                .push(row!()
                    .push(toggle(None, self.enabled)
                        .on_toggle(|v| Message::AgentConfigEnabled(v)))
                    .push(text("Enabled").size(11).color(pal.text))
                    .align_y(Alignment::Center)
                    .spacing(6))
                .push(horizontal_rule(1).padding([8, 0]))
                .push(text("System Prompt").size(11).color(pal.text))
                .push(text_input("", &self.system_prompt)
                    .on_input(|v| Message::AgentConfigSystemPrompt(v))
                    .padding([4, 8]))
                .push(horizontal_rule(1).padding([8, 0]))
                .push(text("Allowed Tools").size(11).color(pal.text))
                .push(row!()
                    .push(button(text("All"))
                        .on_press(Message::AgentConfigSelectAllTools)
                        .padding([2, 8]))
                    .push(button(text("None"))
                        .on_press(Message::AgentConfigClearTools)
                        .padding([2, 8])))
                .push(if self.available_tools.is_empty() {
                    text("No tools registered.").size(11).color(pal.text).into()
                } else {
                    let mut tcol = column!().spacing(2);
                    for (i, tool) in self.available_tools.iter().enumerate() {
                        tcol = tcol.push(
                            row!()
                                .push(toggle(None, self.tool_checkboxes[i])
                                    .on_toggle(move |v| Message::AgentConfigToolToggle(i, v)))
                ...
                .push(row!()
                    .push(button(text("Save"))
                        .on_press(Message::AgentConfigSave)
                        .padding([4, 16]))
                    .push(button(text("Cancel"))
                        .on_press(Message::AgentConfigCancel)
                        .padding([4, 16]))
                    .push(if !self.is_new && self.selected_index >= 0 {
                        row!()
                            .push(button(text("Delete"))
                                .on_press(Message::AgentConfigDelete)
                                .padding([4, 12]))
                            .into()
                    } else {
                        row!().into()
            ...
            column!()
                ...
                .push(text("Select an agent or click \"+ Add Agent\"").size(12).color(pal.text))
                .push(iced::widget::vertical_space())
                .into()
        };
        ...
        let content = column!()
            ...
            .push(row!()
                .push(column!()
                    ...
                    .push(button(text("+ Add Agent"))
                        .on_press(Message::AgentConfigNew)
                        .padding([4, 8]))
                    .push(agent_list)
                    .width(180))
                .push(horizontal_rule(1).padding([8, 0]))
