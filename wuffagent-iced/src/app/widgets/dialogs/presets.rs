...
impl PresetsDialog {
    ...
    pub fn view(&self, pal: Palette) -> Element<'static, Message> {
        ...
        let preset_list = if self.store.presets.is_empty() {
        ...
        let actions = if let Some(i) = self.selected_index {
            let name = self.store.presets[i].name().to_string();
            row!()
                .push(button(text("Load"))
                    .on_press(Message::PresetsLoad(i))
                    .padding([4, 12]))
                .push(button(text("Delete"))
                    .on_press(Message::PresetsDelete(i))
                    .padding([4, 12]))
                .spacing(8)
                .into()
        } else {
        ...
        let add_form = if self.show_add_form {
            column!()
                .push(horizontal_rule(1).padding([4, 0]))
                .push(text("Add New Preset").size(13).color(pal.text))
                .push(text_input("Name", &self.new_name)
                    .on_input(|v| Message::PresetsNewName(v))
                    .padding([4, 8]))
                .push(row!()
                    .push(button(text(if self.new_type == PresetType::Local { "● Local" } else { "○ Local" }))
                        .on_press(Message::PresetsNewType(PresetType::Local))
                        .padding([4, 8]))
                    .push(button(text(if self.new_type == PresetType::Remote { "● Remote" } else { "○ Remote" }))
                        .on_press(Message::PresetsNewType(PresetType::Remote))
                        .padding([4, 8]))
                    .spacing(8))
                .push(if self.new_type == PresetType::Local {
                    column!()
                        .push(text_input("Server path", &self.new_server_path)
                            .on_input(|v| Message::PresetsNewServerPath(v))
                            .padding([4, 8]))
                        .push(text_input("Model path", &self.new_model_path)
                            .on_input(|v| Message::PresetsNewModelPath(v))
                            .padding([4, 8]))
                        .push(text_input("Port", &self.new_port.to_string())
                            .on_input(|v| Message::PresetsNewPort(v))
                            .padding([4, 8]))
                        .push(text_input("GPU layers", &self.new_n_gpu_layers.to_string())
                            .on_input(|v| Message::PresetsNewGpuLayers(v))
                            .padding([4, 8]))
                        .push(text_input("Context size", &self.new_n_ctx.to_string())
                            .on_input(|v| Message::PresetsNewNCtx(v))
                            .padding([4, 8]))
                        .push(text_input("Threads", &self.new_threads.to_string())
                            .on_input(|v| Message::PresetsNewThreads(v))
                            .padding([4, 8]))
                    .into()
                } else {
                    column!()
                        .push(text_input("Remote URL", &self.new_remote_url)
                            .on_input(|v| Message::PresetsNewRemoteUrl(v))
                            .padding([4, 8]))
                        .push(text_input("API Key", &self.new_remote_api_key)
                            .on_input(|v| Message::PresetsNewRemoteApiKey(v))
                            .padding([4, 8]))
                    .into()
                })
                .push(row!()
                    .push(button(text("Add Preset"))
                        .on_press(Message::PresetsAdd)
                        .padding([4, 12]))
                    .push(button(text("Cancel"))
                        .on_press(Message::PresetsCancelAdd)
                        .padding([4, 12]))
                    .spacing(8))
                .into()
        } else {
        ...
        let msg_text = if let Some(ref m) = msg {
            let color = if m.starts_with("Error") {
                pal.danger
            } else {
                pal.success
            };
        ...
        let content = column!()
            ...
            .push(horizontal_rule(1).padding([4, 0]))
            .push(row!()
                .push(button(text("+ New"))
                    .on_press(Message::PresetsNew)
                    .padding([4, 12]))
                .push(iced::widget::horizontal_space())
                .push(button(text("Save Store"))
                    .on_press(Message::PresetsSaveStore)
                    .padding([4, 12])))
            .push(preset_list)
            .push(horizontal_rule(1).padding([4, 0]))
            .push(actions)
